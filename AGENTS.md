...
internal/containers/    container discovery (shells out to the local docker CLI)
...
- **Never commit a `replace ../symaira-corekit`** in go.mod — it breaks CI. Pin
  the published version (currently `v0.5.0`).
...
- `client/Package.swift` exposes **SymscopeFeature** (views + AppState, no
  ...
  CLI JSON contract (currently `1`, since `symscope version --json` shipped).
